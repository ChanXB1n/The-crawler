...
import codecs
import re
writer = codecs.open("jd.csv", "w", "utf-8-sig")
...
class JDCommentsCrawler:
    ...
    def save_csv(self, df, p):
        # 保存文件
        df.to_csv('jd.csv', mode='a', header=False, index=False)
...
def jdComment():
    # 设置关键变量
    page = 3  # 页数
    productId = 12403048  # 商品ID
    callback = 'fetchJSON_comment98vv836'  # 回调函数
    writer.write("comment_id,product_id,guid,content,create_time,reference_id,reference_time,score,nickname,user_level,is_mobile,user_client\n")
    writer.close()
